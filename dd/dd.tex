\documentclass[a4paper,12pt]{report}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[english]{babel}
\usepackage{lmodern}

%Figure progressive enumeration
\usepackage{chngcntr}
\counterwithin{figure}{chapter}
\counterwithin{table}{chapter}

%package used to enumerate figures
\usepackage[labelfont=bf]{caption}

%hyperref for interactive PDF index
\usepackage[bookmarks, colorlinks, breaklinks]{hyperref}
\hypersetup{linkcolor=black, citecolor=black, filecolor=black, urlcolor=black}

%Package required to use special symbols
\usepackage{amsmath, amssymb}

%Package required to use figures
\usepackage{graphicx}
%Include the bibliography in the table of contents
\usepackage{tocbibind}

%Package used to insert figures at the specified position
\usepackage{float}

%Our chapters must be called sections
\addto\captionsenglish{\renewcommand{\chaptername}{Section}}

\begin{document}

%Code for title page
\begin{titlepage}
\centering
\includegraphics[width=0.20\textwidth]{./pictures/logo_poli}\par
	\vspace{1.5cm}
	{\Large {PowerEnJoy \\ 
		Software Engineering II} \par}
	\vspace{1.5cm}
	{\LARGE \textbf{Design Document} \par}
	\vspace{1.5cm}
	{\Large\itshape Giovanni Scotti, Marco Trabucchi\par}
	\vspace{2cm}
	\vfill
	% Bottom of the page
	{\large Document version: 2\par}
	{\large \today \par}
\end{titlepage}

%Make the table of contents
\tableofcontents

%INTRODUCTION
\chapter{Introduction}
\label{ch:Introduction}

\section{Purpose}
\input{./introduction/purpose.tex}

\section{Scope}
\input{./introduction/scope.tex}

\section{Definitions, Acronyms, Abbreviations}
\input{./introduction/def_acr_abbs.tex}

\section{Reference Documents}
\input{./introduction/reference_documents.tex}

\section{Document Structure}
\input{./introduction/document_structure.tex}

\chapter{Architectural Design}
\label{ch:Arch_Design}

\section{Overview}
\input{./arch_design/overview.tex}

\newpage
\section{High Level Components}\label{high_level_comps}
\input{./arch_design/high_level_comps.tex}

\section{Component View}\label{component_view}
\input{./arch_design/component_view.tex}

\section{Deployment View}\label{deployment_view}
\input{./arch_design/deployment_view.tex}

\section{Runtime View}\label{runtime_view}
\input{./arch_design/runtime_view.tex}

\section{Component Interfaces}\label{component_if}
\input{./arch_design/component_interfaces.tex}

\section{Architectural Styles and Patterns}\label{sel_arch_styles_patterns}
\input{./arch_design/architectural_styles.tex}

\section{Other Decisions}\label{other_decisions}
\input{./arch_design/other_decisions.tex}

\chapter{Algorithm Design}
\label{algorithm_design}

\section{Discounts and Additional Charges Management}
\input{./algorithm_design/discounts_algorithm.tex}

\chapter{User Interface Design}
\label{ui_design}

\section{UX Diagrams}
\input{./user_interface_design/ux_diagrams.tex}

\section{User Interface}
\input{./user_interface_design/user_interface.tex}

\chapter{Requirements Traceability}
\label{requirements_traceability}

\section{Functional Requirements}
\input{./requirements_traceability/functional_requirements.tex}

\section{Non-Functional Requirements}
\input{./requirements_traceability/non_functional_requirements.tex}

\appendix
\chapter{Appendix}
\input{./appendix.tex}

\input{./bibliography.tex}

\end{document}