\documentclass[a4paper,12pt]{report}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[english]{babel}
\usepackage{lmodern}

%Figure progressive enumeration
\usepackage{chngcntr}
\counterwithin{figure}{chapter}
\counterwithin{table}{chapter}

%package used to enumerate figures
\usepackage[labelfont=bf]{caption}

%hyperref for interactive PDF index
\usepackage[bookmarks, colorlinks, breaklinks]{hyperref}
\hypersetup{linkcolor=black, citecolor=black, filecolor=black, urlcolor=black}

%Package required to use special symbols
\usepackage{amsmath, amssymb}

%Package required to use figures
\usepackage{graphicx}
%Include the bibliography in the table of contents
\usepackage{tocbibind}

%Package used to insert figures at the specified position
\usepackage{float}

%Packages for Alloy code listing
\usepackage{listings}
\usepackage{alloy}
\usepackage{color}
\definecolor{alloy-keyword}{rgb}{0.23, 0.23, 0.7}
\definecolor{alloy-comment}{rgb}{0.18, 0.64, 0.18}
\definecolor{alloy-string}{rgb}{0.71, 0.18, 0.71}

%Our chapters must be called sections
\addto\captionsenglish{\renewcommand{\chaptername}{Section}}

\begin{document}

%Code for title page
\begin{titlepage}
\centering
\includegraphics[width=0.20\textwidth]{./pictures/logo_poli}\par
	\vspace{1.5cm}
	{\Large {PowerEnJoy \\ 
		Software Engineering II} \par}
	\vspace{1.5cm}
	{\LARGE \textbf{Requirements Analysis and Specification Document} \par}
	\vspace{1.5cm}
	{\Large\itshape Giovanni Scotti, Marco Trabucchi\par}
	\vspace{2cm}
	\vfill
	% Bottom of the page
	{\large Document version: 3\par}
	{\large \today \par}
\end{titlepage}

%Make the table of contents
\tableofcontents

%INTRODUCTION
\chapter{Introduction}
\label{ch:Introduction}

\section{Purpose}
\input{./introduction/purpose.tex}

\section{Scope}
\input{./introduction/scope.tex}

\section{Goals}
\input{./introduction/goals.tex}

\section{Definitions, Acronyms and Abbreviations}
\input{./introduction/def_acr_abbs.tex}

\section{References}
\input{./introduction/references.tex}

\section{Overview}
\input{./introduction/overview.tex}

\chapter{Overall Description}
\label{ch:Overall_Description}

\section{Product Perspective}
\input{./overall_description/product_perspective.tex}

\section{Product Functions}
\input{./overall_description/product_functions.tex}

\section{User Characteristics}
\input{./overall_description/user_characteristics.tex}

\section{Constraints}
\input{./overall_description/constraints.tex}

\section{Assumptions and Dependencies}
\input{./overall_description/assumptions_dependencies.tex}

\section{Future Extensions}
\input{./overall_description/future_extensions.tex}

\section{World and Machine model interpretation}
\input{./overall_description/world_and_machine.tex}

\chapter{Specific Requirements}
\label{ch:Specific_Requirements}

\section{External Interface Requirements}
\input{./specific_requirements/ext_int_requirements.tex}

\section{Functional Requirements} \label{fun_req}
\input{./specific_requirements/functional_requirements.tex}

\section{Performance Requirements} \label{per_req}
\input{./specific_requirements/performance_requirements.tex}

\section{Software System Attributes} \label{software_sys_attr}
\input{./specific_requirements/software_sys_attr.tex}

\section{Alloy} \label{alloy}
\input{./specific_requirements/alloy/alloy.tex}

\appendix
\chapter{Appendix}
\input{./appendix.tex}

\input{./bibliography.tex}

\end{document}